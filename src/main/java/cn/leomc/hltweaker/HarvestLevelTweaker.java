...
public class HarvestLevelTweaker {
    ...
    private static void onItemTooltip(ItemTooltipEvent event) {
        if (event.getEntity() == null || event.getFlags().isAdvanced() || Screen.hasShiftDown()) {
            Tier tier = null;
            ...
            if (tier != null)
                event.getToolTip().add(Component.translatable("tooltip.hltweaker.harvest_level", Utils.getTierName(tier))
                        .withStyle(ChatFormatting.YELLOW));

            if (Utils.isItemOverridden(event.getItemStack().getItem())) {
                String s = HLTConfig.getItemOverrides().get(rl).left().stream()
                        .map(TagKey::location)
                        .map(id -> {
                ...
                event.getToolTip().add(Component.translatable("tooltip.hltweaker.tool_type", s)
                        .withStyle(ChatFormatting.YELLOW));
            }
            ...
            if (event.getItemStack().getItem() instanceof BlockItem item) {
                ...
                for (HLTTier t : HLTConfig.getLevelsList())
                    if (state.is(t.getTag()))
                        event.getToolTip().add(Component.translatable("tooltip.hltweaker.block_harvest_level", Utils.getTierName(t))
                                .withStyle(ChatFormatting.YELLOW));
            }
        }
    }
